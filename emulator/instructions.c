...
void trk8_adc(trk8_machine_t* machine, uint8_t operands_type) {
    ...
    uint8_t left_operand_id = memory_fetch_byte(
        *machine->memory,
        memory_get_program_counter(*machine->memory)
    ); // the left operand will always be a register, so this is fine

    uint8_t left_operand_value = registers_get(*machine->registers, left_operand_id);
    uint8_t right_operand_value;

    uint8_t right_operand_byte = memory_fetch_byte(
        *machine->memory,
        memory_get_program_counter(*machine->memory)
    );
    ...
    switch (operands_type) {
        case TRK8_OPERANDS_TYPE_REG_REG: {
            right_operand_value = registers_get(*machine->registers, right_operand_byte);
    ...
    uint16_t sum = left_operand_value + right_operand_value + TRK8_BIT_CHECK(flags, TRK8_CARRY_VALUE_BIT);
